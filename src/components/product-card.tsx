  ...
  return (
    <>
        ...
        <CardContent className="p-2 sm:p-3 flex-1 flex flex-col">
          ...
          {showUpdateButton ? (
            <button
              ...
              className="w-full py-1.5 text-[10px] sm:text-xs bg-primary/10 text-primary rounded-md font-bold hover:bg-primary hover:text-white transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {updateButtonLabel}
            </button>
          ) : null}
      ...
      <ProductPriceDialog
        ...
        onSave={handleSubmitPrice}
        confirmLabel={submitProductPrice.isPending ? 'Submitting...' : 'Submit Price'}
        disableSave={!newPrice || Number.parseFloat(newPrice) <= 0 || !item.marketId}
        saving={submitProductPrice.isPending}
      />
    </>
  );
}
